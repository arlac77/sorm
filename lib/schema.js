...
var Table = function(name,attributes,constraints) {
    ...
    this.prepare_update = function(db,attributes) {
        var sql = "UPDATE " + this.name + " SET " +  attributes.join(',') +  " WHERE " + this.pk_predicate();
...
var Schema = function(options) {
    ...
    this.migrate_data = function(db,data_sets) {
        for (var i in data_sets) {
            ...
            t.prepare_update(db,data.attributes);
            ...
            var $insert = function(values) {
                db.get(sql, values, function(error) {
                    if(error) {
                        console.log("error inserting into " + data.table + " \"" + values.join('","') + "\"");
                        console.log(error);
                    }
                });
            };
