...
var constraints = [
	{ regex: /^primary\s+key\s*(.*)/i,   name: "PRIMARY KEY" },
	{ regex: /^not\s+null\s*(.*)/i,      name: "NOT NULL" },
	{ regex: /^null\s*(.*)/i,            name: "NULL" }
	];
...
function tables_from_db(db,callback)
{
    db.all("SELECT name,sql FROM sqlite_master WHERE type='table'",function(error, rows) {
        ...
        for (var i in rows) {
			...
			var m = row.sql.match(/CREATE\s+TABLE\s+((\"([^\"]+)\")|([A-Za-z][A-Za-z0-9_]*))\s*\((.*)/);			
			if(m) {
				var attributes = [];
				var name = m[3] ? m[3] : m[4];
				var remaining = m[5].match(/(.*)\)$/)[1];
				var as = remaining.split(/\s*,\s*/);

				for(var j in as) {
					m = as[j].match(/^([A-Za-z][A-Za-z_0-9]*)\s+([^\s]+)\s+(.*)/);
					attributes.push(new Attribute(m[1],m[2],m[3]));
				}

				//console.log("name: " + name + " " + attributes.join(':'));
