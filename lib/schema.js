...
create_constraint({
	...
	parse: function(matches,cs,constraint) {
		var value = matches[2] ? matches[2] : (matches[3] ? matches[3] : matches[4]);
		var c = Constraint(constraint, { value : value } );
		cs.push(c);
		return matches[5];
	}
});
...
create_constraint({
	...
	ddl_statement: function() { return "CONSTRAINT " + this.name + " " + this.value; },
	parse: function(matches,cs,constraint) {
		matches[1] = matches[1].replace(/\s+$/,'');
		cs.push(Constraint(constraint, { name: unquote(matches[1]), value: matches[6] } ));
		return '';
	}
});
