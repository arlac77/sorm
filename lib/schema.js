    ...
    db.all("SELECT name,sql FROM sqlite_master WHERE type='table'",function(error, rows) {
        ...
        for (var i in rows) {
			...
			if(m) {
				...
				tables.push(Table(name, attributes, constraints));
			}
		}
...
var _Schema = Object.create({
	"load" : function(cb) {
		...
		fs.readFile(this.schema_json_file,function(error,data) {
			...
			var tables = [];
			for(var i in schema.tables) {
				var t = schema.tables[i];
				tables.push(Table(t.name,t.attributes,t.constraints));
			}

			Object.defineProperties(t, {
				"version" : { value : schema.version, enumerable : true },
				"name" : { value : schema.name, enumerable : true },
				"tables" : { value : tables, enumerable : true },
				"migrations" : { value : schema.migrations }
				});
	...
	"name" : { value : "undefined", enumerable : true, configurable : true },
...
var Schema = function(options) {
	...
	return Object.create(_Schema,{
		"name" : { value : options.name, enumerable : true, configurable : true },
		"basedir" : { value : basedir },
		"tables" : { value : tables, enumerable : true, configurable : true }
		});
};
