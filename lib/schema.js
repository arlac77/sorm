				...
				do {
					if(parse_constraints(ps, constraints)) {
						...
						if(m) {
							...
							winston.error("Unknown table ddl content", { input: ps.input });
							break;
						}
					}
				}
...
var RootSchema = Object.create({
	...
	"load_from_object" : function(object) {
		...
		if(!this.versions) {
			this.versions = {};
			this.versions[this.schemaHash] = { tag: 1 };
		}
	},
	...
	"exec_ddl" : function(db,createOptions,callback) {
		...
		for(var j in create_tables) {
			t = create_tables[j];
			winston.info("ddl", { sql: t.ddl_statement()});			
