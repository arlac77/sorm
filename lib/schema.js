...
var constraints = [
	{ regex: /^primary\s+key\s*(.*)/im,   name: "PRIMARY KEY" },
	{ regex: /^not\s+null\s*(.*)/im,      name: "NOT NULL" },
	{ regex: /^null\s*(.*)/im,            name: "NULL" },
	{ regex: /^default\s+(('[^']*')|null)(.*)/im, name: "DEFAULT" }
	];
...
function tables_from_db(db,callback)
{
    db.all("SELECT name,sql FROM sqlite_master WHERE type='table'",function(error, rows) {
        ...
        for (var i in rows) {
			...
			var m = sql.match(/CREATE\s+TABLE\s+((\"([^\"]+)\")|([A-Za-z][A-Za-z0-9_]*))\s*\((.*)/im);			
			if(m) {
				var attributes = [];
