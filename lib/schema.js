...
create_constraint({
	...
	//initialize: function(properties) { if(properties.options) this.options = properties.options.toUpperCase(); },
	ddl_statement: function() { return this.options ? this.name + " " + this.options.toUpperCase() : this.name; },
	parse: function(matches,cs,constraint) {
		var options = matches[2];
		var c = options ? Constraint(constraint, { options : options }) : Constraint(constraint);
		cs.push(c);
		return matches[3];
	}
});
...
var RootSchema = Object.create({
	...
	"exec_ddl" : function(db,createOptions,callback) {
		...
		tables_from_db(db, function(error, present_tables) {
            ...
            for(var i in present_tables) {
				...
				console.log(i + " ::: " + pt.name);
				console.log(i + " ::: " + pt.ddl_statement());
		...
		for(var j in create_tables) {
			t = create_tables[j];
			/*
			console.log("ddl 1: " + t.name);
			console.log("ddl 2: " + t.attributes);
			console.log("ddl 3: " + JSON.stringify(t.constraints));
			console.log("ddl 4: " + t.ddl_statement());
			*/
	...
	"schemaHash" : { get : function() {
        ...
        for (var j in this.tables) {
			console.log(j + " : " + JSON.stringify(this.tables[j]));
