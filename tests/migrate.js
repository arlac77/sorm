...
vows.describe('Schema From File').addBatch({
  'Schema Values': {
    topic() {
        Schema("tests/test2.schema", this.callback);
      },
...
vows.describe('Migration').addBatch({
    'Add Table': {
      topic() {
          Schema("tests/test3.schema", this.callback);
        },
        'hash': function (error, schema) {
          assert.ifError(error);
  ...
  .addBatch({
    'Schema Migration Alter Table': {
      topic() {
          Schema("tests/test4.schema", this.callback);
        },
        ...
        'create database with schema': {
          topic(schema) {
              const db = new sqlite3.Database("tests/test2.db");
              schema.exec_ddl(db, {}, this.callback);
            },
            'create schema in database': function (error, schema, db) {
              if (error) {
